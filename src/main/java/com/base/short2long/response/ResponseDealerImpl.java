...
public class ResponseDealerImpl implements ResponseDealer {
    ...
    public void handleMessage(NettyMessage nettyMessage) {
        try {
            ...
            if (nettyMessage.getBytesIn().length > Constants.CHANNEL_IDX_LEN) {
                byte[] byteIdx = new byte[Constants.CHANNEL_IDX_LEN];
                ...
                if (!Objects.isNull(requestChannel)) {
                    ...
                    //返回
                    System.arraycopy(nettyMessage.getBytesIn(), Constants.CHANNEL_IDX_LEN, byteOut, 0, nettyMessage.getBytesIn().length - Constants.CHANNEL_IDX_LEN);
